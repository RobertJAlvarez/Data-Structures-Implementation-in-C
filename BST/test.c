...
int main(int argc, char **argv)
{
  ...
  for (;;) {
    ...
    if (temp_value != NULL) {
      ...
      search_tree_insert(tree, key, value, compare_key, copy_key, copy_value, NULL);
    }
    ...
    if (temp_value != NULL) {
      ...
      search_tree_predecessor((void **) &temp_key, (void **) &temp_value, tree, key, compare_key, NULL);
      if ((temp_key == NULL) || (temp_value == NULL)) {
      ...
      search_tree_successor((void **) &temp_key, (void **) &temp_value, tree, key, compare_key, NULL);
      if ((temp_key == NULL) || (temp_value == NULL)) {
    ...
    printf("Please enter a key to delete from the tree. Enter <nothing> to delete nothing.\n");
    input_string(key, sizeof(key));
    if (strcmp(key, "<nothing>") != 0) {
      search_tree_remove(tree, key, compare_key, delete_key, delete_value, NULL);
    }
  }
