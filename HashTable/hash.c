...
uint32_t hash_uint32(uint32_t a) { return hash_uint64((uint64_t)a); }

uint32_t hash_int32(int32_t a) { return hash_int64((int64_t)a); }

uint32_t hash_uint16(uint16_t a) { return hash_uint64((uint64_t)a); }

uint32_t hash_int16(int16_t a) { return hash_int64((int64_t)a); }

uint32_t hash_uint8(uint8_t a) { return hash_uint64((uint64_t)a); }

uint32_t hash_int8(int8_t a) { return hash_int64((int64_t)a); }
...
uint32_t hash_float(float a) { return hash_double((double)a); }

static inline uint32_t hash_mem_up_to_8(const void *ptr, const size_t n) {
  uint64_t t;
...
uint32_t hash_mem(const void *ptr, size_t n) {
  ...
  if (n <= ((size_t)8)) return hash_mem_up_to_8(ptr, n);
  ...
  for (i = ((size_t)0); i < w; i++) {
    tt =
        hash_mem_up_to_8(&(((char *)ptr)[(i + ((size_t)1)) << 3]), ((size_t)8));
    t = hash_combine(t, tt);
  }
  if (r > ((size_t)0)) {
...
uint32_t hash_str(const char *ptr) { return hash_mem(ptr, strlen(ptr)); }
